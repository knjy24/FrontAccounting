...
function getTransactions($from, $to)
{
	...
	$sql = "SELECT a.*, DATE_FORMAT(a.tran_date, '%m-%d-%Y') as tranDate, c.*, d.*, v.type as IsVoid from ".TB_PREF."debtor_trans a 
			LEFT JOIN ".TB_PREF."voided v  ON a.type = v.type and a.trans_no=v.id
	...
	if ($fromdate != '')
		$sql .= " AND a.tran_date >= '$fromdate' and a.tran_date <= '$todate'";

	$sql .= " ORDER BY d.customized_no";
...
function print_cash_sale()
{
	...
	while ($myrow=db_fetch($res))
	{
		...
		$returnTotal = $returns + $rtn_dt;
			$net_sales = abs($sales_acct) - $dt - $rtn_dt;
			if ($net_sales == 0)
				$rep->TextCol(6,7, "Sales account for this client not set.");
			else
				$rep->AmountCol(6,7, $net_sales, 2);
			if ($rtn_dt != 0)
			$rep->TextCol(7,8, $returnTotal);
			if ($dt != 0)
			...
			$rep->TextCol(1,2, $myrow['br_name']." -- CANCELLED");
			$rep->NewLine();
