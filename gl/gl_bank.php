...
include_once($path_to_root . "/gl/ml/db/ml_gl_trans.inc"); //moodlearning
include_once($path_to_root . "/reporting/includes/reporting.inc"); //moodlearning
...
function create_cart($type, $trans_no)
{
	...
	$_POST['address'] = $cart->address; //moodlearning
	$_POST['check_num'] = $cart->check_num;//moodlearning
...
/*=========================MOODLEARNING===========================*/
	if ($_SESSION['pay_items']->trans_type == ST_BANKPAYMENT)
	{
		if (!get_post('cv_no'))
		{
			display_error(_("Please enter a Check Voucher number."));
			set_focus('cv_no');
			$input_error = 1;
		} else 
		{
			if (checkNoIfExists($_POST['cv_no']) > 0){
				display_error(_("Check Voucher Number already exists. Please check again."));
				set_focus('cv_no');
				$input_error = 1;
			}
		}	
...
if (isset($_POST['Process']) && !check_trans())
{
	...
	$trans_type = $trans[0];
   	...
   	write_trans($_SESSION['pay_items']->trans_type, $trans_no, $_POST['cv_no'], $_POST['address'], $_POST['check_num']); //moodlearning
