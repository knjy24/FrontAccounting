...
$path_to_root = "..";
include_once($path_to_root . "/includes/ui/custom_allocation_cart.inc");
...
include_once($path_to_root . "/purchasing/includes/ui/custom_ui.inc");
...
add_js_file('custom_payalloc.js');
		...
		if($inv) {
			...
			foreach($_SESSION['alloc']->allocs as $line => $trans) {
				if ($trans->type == ST_SUPPINVOICE && $trans->type_no == $_GET['PInvoice']) {
					$un_allocated = abs($trans->payment) - $trans->amount_allocated;
					$_SESSION['alloc']->payment = $_SESSION['alloc']->allocs[$line]->current_allocated = $un_allocated;
					$_POST['payment'] = $_POST['payment'.$line] = price_format($un_allocated);
					break;
				}
			}
			unset($inv);
...
if (isset($_GET['AddedID'])) {
	$payment_id = $_GET['AddedID'];
   	...
   	submenu_print(_("&Print This Voucher"), ST_DISBURSEMENT, $payment_id."-".ST_SUPPAYMENT, 'prtopt');
...
if (isset($_POST['AddGLCodeToTrans'])){
	...
	{
		$_SESSION['alloc']->add_gl_codes_to_trans($_POST['gl_code'], $gl_act_name,
...
function check_inputs()
{
	...
	if (($limit !== null) && (floatcmp($limit, input_num('payment')) < 0))
	{
	...
	$_SESSION['alloc']->payment = -input_num('payment');
...
function handle_add_payment()
{
	$payment_id = write_supp_payment(0, $_POST['supplier_id'], $_POST['bank_account'],
		$_POST['DatePaid'], $_POST['ref'], input_num('payment'),	input_num('discount'), $_POST['memo_'], 
		input_num('charge'), input_num('bank_amount', input_num('payment')), $_POST['custom_num'], $_SESSION['alloc']);//moodleanring custom_num
	...
	$_SESSION['alloc']->write();
   	...
   	unset($_POST['custom_num']);//moodlearning
...
if ($id != -1)
{
	$_SESSION['alloc']->remove_gl_codes_from_trans($id);
	clear_fields();
	reset_tot();
	$Ajax->activate('gl_items');
}
...
{
	$Ajax->activate('gl_items');
	display_quick_entries($_SESSION['alloc'], $_POST['qid'], input_num('totamount'), QE_SUPPINV);
...
start_form();
    ...
    //========MOODLEARNING=============
    text_row(_("Check No."), 'custom_num', $_POST['custom_num'], 20,60);
