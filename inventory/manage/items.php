...
include_once($path_to_root . "/inventory/author/includes/author_db.inc");//ML
include_once($path_to_root . "/inventory/author/includes/author_ui.inc");//ML

/*=======================MOODLEARNING=================*/
include_once($path_to_root . "/inventory/ml/ui/trans_ui.inc");
/*====================================================*/
...
function clear_data()
{
	...
	unset($_POST['level_id']); // MOODLEARNING EDIT
...
function item_settings(&$stock_id) 
{
	...
	if ($new_item && (list_updated('category_id') || !isset($_POST['units']))) {
		...
		$_POST['level_id'] = $category_record["level_id"]; // moodlearning edit
...
tabbed_content_start('tabs', array(
		...
		'author' => array(_('Authors && Payments'), $stock_id),//ML
	...
	switch (get_post('_tabs_sel')) {
		...
		case 'status':
			...
			break;
		case 'author'://ML
			$_GET['stock_id'] = $stock_id;//ML
			$_GET['popup'] = 1;//ML
			include_once($path_to_root."/inventory/author/royalty_author.php");//ML
			break;//ML
