...
page(_($help_context = "Book Subject"));
...
if ($Mode=='ADD_ITEM') 
{
	$checkIfExists = check_item_subject($_POST['description']);
	$input_error = 0;
	...
	if ($input_error != 1)
	{
    	if ($selected_id != -1) 
    	{
    		update_item_subject($selected_id, $_POST['description']);
			$note = _('Selected book subject has been updated');
    	} 
    	else 
    	{
    		add_item_subject($_POST['description']);
			$note = _('New book subject has been added');
    	}
...
if ($Mode == 'Delete')
{
	...
	if ($cancel_delete == 0) 
	{
		delete_item_subject($selected_id);
