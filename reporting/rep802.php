...
function fetchRoyalty($id){
    $sql = "SELECT d.br_name, a.customized_no, b.tran_date, c.quantity, c.stock_id FROM ".TB_PREF."customized as a INNER JOIN ".TB_PREF."debtor_trans as b ON a.type_no = b.trans_no INNER JOIN ".TB_PREF."debtor_trans_details as c ON b.trans_no = c.debtor_trans_no INNER JOIN ".TB_PREF."cust_branch as d ON b.debtor_no = d.branch_code WHERE a.type = 10 AND b.type = 10 AND c.debtor_trans_type = 10 AND b.ov_amount = b.alloc AND c.stock_id = ".db_escape($id)." ";
    return db_query($sql, 'error');
}

function fetchCreditMemo($id){
    $sql = "SELECT d.br_name, a.customized_no, b.tran_date, c.quantity, c.stock_id FROM ".TB_PREF."customized as a INNER JOIN ".TB_PREF."debtor_trans as b ON a.type_no = b.trans_no INNER JOIN ".TB_PREF."debtor_trans_details as c ON b.trans_no = c.debtor_trans_no INNER JOIN ".TB_PREF."cust_branch as d ON b.debtor_no = d.branch_code WHERE a.type = 11 AND b.type = 11 AND c.debtor_trans_type = 11 AND b.ov_amount = b.alloc AND c.stock_id = ".db_escape($id)." ";
    return db_query($sql, 'error');
}
