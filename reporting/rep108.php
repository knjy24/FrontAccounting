...
function getTransactions($debtorno, $from,  $date)
{
	$sql = "SELECT ".TB_PREF."debtor_trans.*, v.type as IsVoid,	
		...
		CASE WHEN ".TB_PREF."debtor_trans.type=".ST_SALESINVOICE." 
			THEN (".TB_PREF."debtor_trans.ov_amount != ".TB_PREF."debtor_trans.alloc)
			else
				(".TB_PREF."debtor_trans.ov_amount = ".TB_PREF."debtor_trans.alloc OR ".TB_PREF."debtor_trans.ov_amount != ".TB_PREF."debtor_trans.alloc)
			END";
...
function print_statements()
{
	...
	curdate() AS tran_date, d.name as Name1, d.name2 as Name2, d.phone, d.phone2  
...
INNER JOIN ".TB_PREF."cust_branch cust on b.debtor_no=cust.debtor_no";
	if ($customer != ALL_TEXT)
		$sql .= " WHERE c.type='customer' and b.debtor_no = ".db_escape($customer);
	else
		$sql .= " where c.type='customer' and b.debtor_no = ".db_escape($customer)." ORDER by b.name";
	...
	while ($debtor_row=db_fetch($result))
	{
		...
		$TransResult = getTransactions($debtor_row['debtor_no'], $from,  $to);
