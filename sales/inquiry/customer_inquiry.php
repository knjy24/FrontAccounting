...
text_cells(_("#:"), 'customNum'); //Edited by Karen 07/09/2015
...
function systype_name($row, $type) 
{
	...
	$var = "Charge Invoice"; //moodlearning edit $row
	if ($row['type'] == ST_SALESINVOICE && $row['payment_terms'] !=4)
		return $var;
	else
...
function prt_link($row)
{
  	...
  	elseif ($row['type'] == ST_BANKPAYMENT) // bank payment printout not defined yet.
		return '';
 	elseif ($row['type'] == ST_SALESINVOICE && $row['payment_terms'] == 4)
 		return print_document_link($row['trans_no']."-".$row['type'], _("Print"), true, ST_CASHSALE, ICON_PRINT);
 	else
...
$cols = array(
	...
	_("Ref") => array('fun'=>'trans_view', 'ord'=>''),
	_("Order") => array('fun'=>'order_view'), 
	_("#") => array('fun' => 'custom_view'),
