...
include_once($path_to_root . "/sales/ml/includes/db/customer_trans.php");
...
if (get_post('AddPaymentItem') && can_process()) {
	...
	$payment_no = write_customer_payment($_SESSION['alloc']->trans_no, $_POST['customer_id'], $_POST['BranchID'],
		...
		input_num('amount'), input_num('discount'), $_POST['memo_'], 0, input_num('charge'), input_num('bank_amount', input_num('amount')), $_POST['or_num'], $_POST['or_date']);//moodlearning
...
if (isset($_GET['trans_no']) && $_GET['trans_no'] > 0 )
{
	...
	$_POST['or_num'] = $ornum;//moodlearning
	$_POST['or_date'] = sql2date($ordate);//moodlearning
...
start_form();
	...
	text_row(_("OR #:"), 'or_num', null, ''); //moodlearning

	date_row(_("Date of OR:"), 'or_date', '', true, 0, 0, 0, null, true);//moodlearning
