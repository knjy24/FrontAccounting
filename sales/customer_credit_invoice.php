...
function check_quantities()
{
	...
	foreach ($_SESSION['Items']->line_items as $line_no=>$itm) {
		...
		if (isset($_POST['Line'.$line_no])) {
			...
			// Karen edited
			$tots = $_POST['Line'.$line_no] + $itm->qty_done;
			if ($tots <= $itm->quantity)
			{
				$_SESSION['Items']->line_items[$line_no]->qty_dispatched = input_num('Line'.$line_no);
			}
			else {
				display_error(_($itm->stock_id."-".$itm->item_description));
				$ok = 0;
			}
			//end of edited
...
function copy_from_cart()
{
	...
	$_POST['bulk_dis'] = $cart->bulk_discount; //moodlearning
}
//-----------------------------------------------------------------------------
...
//	sales_types_list_cells(null, 'sales_type_id', $_POST['sales_type_id']);
    ...
    if ($_SESSION['Items']->custom_num != '')
    	text_row(_("Credit Memo No."), 'custom_num', $_SESSION['Items']->custom_num, null, "class='tableheader2'"); //moodlearning
    else
    	text_row(_("Credit Memo No."), 'custom_num', '', null, "class='tableheader2'"); //moodlearning
    ...
    $th = array(_("Item Code"), _("Item Description"), _("Invoiced Quantity"), _("Units"),  _("Credited"), 
    	_("Credit Quantity"), _("Price"), _("Discount %"), _("Total")); //Karen edited Credited
    table_header($th);
    ...
    foreach ($_SESSION['Items']->line_items as $line_no=>$ln_itm) {
    	...
    	label_cell($ln_itm->qty_done); //Karen edited
    ...
    label_row(_("Credit Note Total"), $tot, "colspan=$colspan align=right", "align=right"); //moodlearning edit $tot
