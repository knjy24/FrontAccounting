...
//include_once($path_to_root . "/sales/ml/db/customer_trans.inc"); //moodlearning
...
if (isset($_GET['AddedID'])) {
	...
	submenu_print(_("&Print Sales Invoice (CASH SALES)"), ST_CASHSALE, $invoice."-".ST_SALESINVOICE, 'prtopt');

	submenu_print(_("&Print Charge Invoice w/ Discount"), ST_SALESINVOICE, $invoice."-".ST_SALESINVOICE, 'prtopt');
	//submenu_print(_("&Print Charge Invoice w/o Discount"), ST_CHARGEINVOICE, $invoice."-".ST_SALESINVOICE, 'prtopt');
...
function copy_to_cart()
{
	...
	$cart->custom_num = $_POST['ref']; //moodlearning
	...
	$cart->bulk_discount = input_num('bulk_dis'); //moodlearning
...
function copy_from_cart()
{
	...
	$_POST['bulk_dis'] = price_format($cart->bulk_discount); //moodlearning
...
function can_process() {
	...
	/*if (checkIfInvoiceExists($_SESSION['Items']->trans_type, get_post('custom_num'))){
				display_error(_("Invoice number already exists."));
				set_focus('custom_num');
				return false;
			}*/
...
if ($customer_error == "") {
	start_table(TABLESTYLE, "width=100%", 2);
	echo "<tr><td>";
