...
//---------------------------------------------------------------------------
// edited sunday : customer_invoice.php, cust_trans_db.inc(get_sql)
...
if (isset($_GET['AddedID'])) {
	...
	display_note(get_trans_view_str(ST_SALESINVOICE, $invoice_no, _("&View This Invoice")));
		echo '<br>';
	display_note(print_document_link($invoice_no."-".ST_SALESINVOICE, _("&Print Sales Invoice (CASH SALES)"), true, ST_CASHSALE));
	echo '<br>';
	display_note(print_document_link($invoice_no."-".$trans_type, _("&Print Charge Invoice"), true, ST_SALESINVOICE));
	display_note(print_document_link($invoice_no."-".$trans_type, _("&Email This Invoice"), true, ST_SALESINVOICE, false, "printlink", "", 1),1);
...
|| isset($_GET['BatchInvoice'])) {
	...
	if ( get_sales_parent_numbers(ST_SALESINVOICE, $_GET['ModifyInvoice']) == 0) { // 1.xx compatibility hack
		...
		Most likely this invoice was created in Serbizhub Suite version prior to 2.0
...
function copy_to_cart()
{
	...
	$cart->custom_num = $_POST['custom_num']; //moodlearning
	$cart->bulk_discount = $_POST['bulk_dis']; //moodlearning
...
function copy_from_cart()
{
	...
	$_POST['custom_num'] = $cart->custom_num; //moodlearning
	$_POST['bulk_discount'] = $cart->bulk_discount;
}
...
/*=====================MOODLEARNING=================*/
	if ($_SESSION['Items']->trans_type==ST_SALESINVOICE){
		if(!get_post('custom_num')){
			display_error(_("There is no invoice number entered."));
...
if(list_updated('payment')) {
	...
	$order->custom_num = get_post('customer');
...
text_cells(_("Invoice #"), 'custom_num', $_SESSION['Items']->custom_num, null, "class='tableheader2'"); //moodlearning
...
$ds = input_num('bulk_dis') / 100; //moodlearning
$tots = $inv_items_total + input_num('ChargeFreightCost') + $tax_total; //moodlearning
$display_total = price_format($tots - ($tots * $ds)); //moodlearning
