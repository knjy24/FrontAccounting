...
}else if(cmd=='di'){objElement.disabled=data;}else if(cmd=='fc'){_focus=data;}else if(cmd=='js'){__isGecko ? eval(data):setTimeout(function(){eval(data);},200);}else if(cmd=='rd'){window.location=data;}else if(cmd=='pu'){newwin=1;window.open(data,'REP_WINDOW','toolbar=no,scrollbars=yes,resizable=yes,menubar=no');}else{errors=errors+'<br>Unknown ajax function: '+cmd;}
}
if(tcheck)
...
if(el.type=='file')
q[name]=el;else{q[name]=el.value;}
}
}
}
...
num.substring(num.length-(4*i+3));num=((sign)?'':'-')+num;if(dec!=0&&(!max||cents!=0))
...
function move_focus(dir,e0,neighbours){var p0=element_pos(e0);var t;var l=0;for(var i=0;i<neighbours.length;i++){var e=neighbours[i];var p=element_pos(e);if(p!=null&&(e.className=='menu_option'||e.className=='printlink'
||e.className=='repclass_link'||e.className=='repopts_link')){if(((dir==40)&&(p.y>p0.y))||(dir==38&&(p.y<p0.y))
